...
class MPI(nn.Module):
    ...
    def forward_render(self,
        ...
        with torch.no_grad():
            ...
            weight = torch.stack(weight, 1) # [n_points, n_planes, 4, 1]
        ...
        alpha = rgbd[:,3:].reshape([n_points,n_planes,4,1])
        # [n_points, n_planes, 3] -> [n_points, 3] 
        rays_rgb = (weight * rgb * alpha).sum(dim=[1,2]) / alpha.sum(dim=[1,2])
        # n_planes with alpha value?
    ...
    def get_grid_weight(self, inter_point, p, proj_to_grid):
        ...
        vec_p = torch.zeros_like(h_floor)+(p+1)*100
