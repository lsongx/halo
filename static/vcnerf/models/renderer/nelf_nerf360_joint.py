...
class NeLFNeRF360Joint(nn.Module):
    def __init__(self, 
                 xyz_embedder, 
                 uv_embedder,
                 st_embedder,
                 ...
                 dir_embedder=None,):
        super().__init__()
        self.padding_lf_xyz = xyz_embedder.nb_freqs-lf_xyz_embedder.nb_freqs
        ...
        self.nelf_converge_iter = nelf_converge_iter
    ...
    def forward_render(self, 
        ...
        if self.iter < self.nelf_converge_iter:
            render_args['z_by_nelf'] = -1
            render_args['embedder'] = self.lf_xyz_embedder
    ...
    def forward_points(self, field, embedder, points, directions=None,):
        ...
        if xyz_embeds.shape[-1] != field.layers.fc0.in_features:
            xyz_embeds = torch.cat([xyz_embeds, torch.zeros(
                [points.shape[0],self.padding_lf_xyz*6], device=points.device)], -1)
