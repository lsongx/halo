...
class MaskNeRF(nn.Module):
    ...
    def forward(self, rays, render_params=None):
        ...
        background_mask = rays['rays_empty'][:,0]
        outputs['coarse_empty_loss'] = ((outputs['coarse']['acc_map'][background_mask])**2).mean()*0.1

        if outputs['fine'] is not None:
            im_loss_fine = im2mse(outputs['fine']['color_map'], rays['rays_color'])
            outputs['fine_loss'] = im_loss_fine
            outputs['fine_empty_loss'] = ((outputs['fine']['acc_map'][background_mask])**2).mean()*0.1
    ...
    def forward_render(self,
                       rays_ori, rays_dir, rays_color, rays_empty, # loader output
