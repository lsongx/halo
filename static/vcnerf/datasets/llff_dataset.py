...
class LLFFDataset:
    def __init__(self, 
        ...
        images, poses, bds, render_poses, i_test, imgfiles, mean_z = load_llff_data(
        ...
        self.mean_z = mean_z
        self.all_poses = poses[:, :3, :4]
        self.logger.info(f'render_poses.shape {render_poses.shape}, '
                         f'hwf {self.hwf}, datadir {datadir}')
        self.center_pose_idx = i_test
        ...
        self.poses = torch.tensor(self.all_poses[all_idx])
        ...
        if self.batching:
            ...
            self.all_rays_color = self.imgs.view([-1,3])[self.permute]
