...
H36M_ROOT = os.path.expanduser('~/data/human36m/training/subject/')
UNA_ROOT = os.path.expanduser('~/data/human36m/training/subject/')
BBOX_PATH = os.path.expanduser('~/data/human36m/training/subject/extra/bboxes-Human36M-GT.npy')
...
table_dtype = np.dtype([
    ...
    ('keypoints_full', np.float32, (32,3)), 
...
cameras_params = h5py.File(os.path.join(UNA_ROOT, 'cameras.h5'), 'r')
...
# Fill bounding boxes
bboxes = np.load(BBOX_PATH, allow_pickle=True).item()
...
for subject_idx, subject in enumerate(retval['subject_names']):
    ...
    for action_idx, action in enumerate(retval['action_names']):
        ...
        with h5py.File(os.path.join(UNA_ROOT, subject, 'MyPoses', '3D_positions',
            ...
            poses_world_full = np.array(poses_file['3D_positions']).T.reshape(-1, 32, 3)[frame_idxs]
        ...
        table_segment['keypoints_full'] = poses_world_full
