...
ROOT = path.expanduser('~/data/human36m/training/subject/')
...
def process_view(out_dir, subject, action, subaction, camera):
    subj_dir = path.join(ROOT, subject)

    base_filename = metadata.get_base_filename(subject, action, subaction, camera)

    # Load joint position annotations
    cdf_file = cdflib.CDF(path.join(subj_dir, 'MyPoseFeatures', 'D2_Positions', base_filename + '.cdf'))
    poses_2d = cdf_file.varget('Pose')
    poses_2d = poses_2d.reshape(poses_2d.shape[1], 32, 2)
    cdf_file.close()

    cdf_file = cdflib.CDF(path.join(subj_dir, 'MyPoseFeatures', 'D3_Positions_mono_universal', base_filename + '.cdf'))
    poses_3d_univ = cdf_file.varget('Pose')
    poses_3d_univ = poses_3d_univ.reshape(poses_3d_univ.shape[1], 32, 3)
    cdf_file.close()

    cdf_file = cdflib.CDF(path.join(subj_dir, 'MyPoseFeatures', 'D3_Positions_mono', base_filename + '.cdf'))
    poses_3d = cdf_file.varget('Pose')
...
if __name__ == '__main__':
    # for test
    f = h5py.File(path.join(ROOT, 'processed/S1/Directions-1/annot.h5'), 'r')
