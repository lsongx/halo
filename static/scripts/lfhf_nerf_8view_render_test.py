...
for scene in scene_img.keys():
    ...
    for i in range(num_images):
        ...
        plt.imsave(f'{out}/{scene}-{i}-hf-nerf.png', nerf_im)
        plt.imsave(f'{out}/{scene}-{i}-hf-depth.png', nerf_depth[:,:,0], cmap='Oranges') 


for scene in scene_img.keys():
    path = f'{root}/{scene}-lfnerf'
    for f in os.listdir(path):
        if '.py' in f:
            print(f'{f} loaded.')
            break
    ...
    for i in range(num_images):
        ...
        nelf_aug = result.get('nelf_aug_color_map', result['color_map']).reshape([h,w,-1]).cpu().numpy()
        gt = data['rays_color'].cpu().numpy().reshape([h,w,-1]) # extra dim
        plt.imsave(f'{out}/{scene}-{i}-lf-nerf.png', nerf_im)
        plt.imsave(f'{out}/{scene}-{i}-lf-depth.png', nerf_depth[:,:,0], cmap='Oranges') 
