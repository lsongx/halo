...
while args.k < len(scenes):
    ...
    if os.path.isdir(f'./data/stanford_epi_4view_nolf/{scene}'):
        os.system(f'rm -r ./data/stanford_epi_4view_nolf/{scene}')
    ...
    subprocess.run(f"python -m torch.distributed.launch --nproc_per_node=1 train.py "
                   ...
                   f"--work_dir ./data/stanford_epi_4view_nolf/{scene} --port {25688+args.k} "
                   f"--cfg-options data.train.dataset.base_dir='~/data/3d/StanfordLF/{scene}/rectified' "
                   f"data.val.base_dir='~/data/3d/StanfordLF/{scene}/rectified' "
                   f"model.epi_converge_range='1' "+scenes[scene],
                   shell=True)
    
    for f in os.listdir(f'./data/stanford_epi_4view_nolf/{scene}'):
        if '.log.json' in f:
            with open(f'./data/stanford_epi_4view_nolf/{scene}/{f}', 'r') as tf:
                val_logs = [json.loads(line) for line in tf.read().split('\n') if '"mode": "val"' in line]
            break
    all_psnr = torch.tensor([k['psnr'] for k in val_logs])
    idx = all_psnr.argmax()
    
    if os.path.isfile(f'./data/stanford_epi_4view_nolf/all_stanfordlf.json'):
        with open(f'./data/stanford_epi_4view_nolf/all_stanfordlf.json', 'r') as f:
    ...
    with open(f'./data/stanford_epi_4view_nolf/all_stanfordlf.json', 'w') as f:
    ...
    with open(f'./data/stanford_epi_4view_nolf/mean.log', 'w') as f:
