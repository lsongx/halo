...
while args.k < len(scenes_train):
    ...
    if os.path.isdir(f'./data/llff-halo-nolf/{scene}'):
        os.system(f'rm -r ./data/llff-halo-nolf/{scene}')
    ...
    subprocess.run(f"python -m torch.distributed.launch --nproc_per_node=1 train.py "
                   ...
                   f"--work_dir ./data/llff-halo-nolf/{scene} --port {25888+args.k} "
                   ...
                   f"data.val.datadir='~/data/3d/nerf/nerf_llff_data/{scene}' model.epi_converge_range='1' ",
                   shell=True)

    for f in os.listdir(f'./data/llff-halo-nolf/{scene}'):
        if '.log.json' in f:
            with open(f'./data/llff-halo-nolf/{scene}/{f}', 'r') as tf:
                val_logs = [json.loads(line) for line in tf.read().split('\n') if '"mode": "val"' in line]
            break
    all_psnr = torch.tensor([k['psnr'] for k in val_logs])
    idx = all_psnr.argmax()
    
    if os.path.isfile(f'./data/llff-halo-nolf/all_llff.json'):
        with open(f'./data/llff-halo-nolf/all_llff.json', 'r') as f:
    ...
    with open(f'./data/llff-halo-nolf/all_llff.json', 'w') as f:
    ...
    with open(f'./data/llff-halo-nolf/mean.log', 'w') as f:
