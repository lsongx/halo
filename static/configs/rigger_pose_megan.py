...
model = dict(
    ...
    #     use_dirs=False,
    #     use_sin=False),
    pose_deform_field=dict(
        type='PoseDeformField',
        nb_layers=8, 
        hid_dims=256, 
        xyz_emb_dims=63, # 2*3*10+3
        use_sin=False),
...
# learning policy
lr_config = dict(policy='Exp', gamma=0.1**(1/25000), by_epoch=False) 
# lr_config = dict(policy='Cosine', min_lr=5e-5, by_epoch=False) 
