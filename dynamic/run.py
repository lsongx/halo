...
def config_parser():
    ...
    parser.add_argument("--eval_lpips_alex", type=int, default=1)
    parser.add_argument("--eval_lpips_vgg", type=int, default=1)
    parser.add_argument("--eval_psnr", type=int, default=1)
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    if savedir is not None:
        ...
        for i in trange(len(depths)):
            plt.imsave(os.path.join(savedir,f'{i:03d}depth.png'), depths[i][...,0], cmap='turbo')
            color_depths.append((plt.imread(os.path.join(savedir,f'{i:03d}depth.png'))*255.).astype('uint8'))

    rgbs = np.array(rgbs)
    # depths = np.array(depths)
    depths = np.stack(color_depths, 0)
    return rgbs, depths
...
if __name__=='__main__':
    ...
    if args.render_train:
        ...
        imageio.mimwrite(os.path.join(testsavedir, '_train_video.rgb.mp4'), utils.to8b(rgbs), fps = 30, quality = 8)
        imageio.mimwrite(os.path.join(testsavedir, '_train_video.disp.mp4'), utils.to8b(disps / np.max(disps)), fps = 30, quality = 8)
    ...
    if args.render_test:
        ...
        imageio.mimwrite(os.path.join(testsavedir, '_test_video.rgb.mp4'), utils.to8b(rgbs), fps=30, quality=8)
        imageio.mimwrite(os.path.join(testsavedir, '_test_video.disp.mp4'), utils.to8b(disps / np.max(disps)), fps=30, quality=8)
    ...
    if args.render_video:
        if cfg.data.dataset_type  != 'hyper_dataset':
            ...
            rgbs, disps = render_viewpoints(
                    ...
                    **render_viewpoints_kwargs)
            imageio.mimwrite(os.path.join(testsavedir, '_video.rgb.mp4'), utils.to8b(rgbs), fps=30, quality=8)
            imageio.mimwrite(os.path.join(testsavedir, '_video.disp.mp4'), utils.to8b(disps / np.max(disps)), fps=30, quality =8)
        else:
            raise NotImplementedError
